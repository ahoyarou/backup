...
// Спроба отримати плагін ShowHiddenChannels
let ShowHiddenChannels: any = null;
try {
    const plugins = require("@plugins");
    ShowHiddenChannels = plugins.plugins?.ShowHiddenChannels;
} catch (e) {
    // Ігноруємо, якщо плагін не знайдено
}
...
// Функція для перевірки, чи є канал прихованим
function isHiddenChannel(channel: Channel | null | undefined): boolean {
    if (!channel) return false;
    try {
        // Перевіряємо, чи існує плагін ShowHiddenChannels та його метод isHiddenChannel
        if (ShowHiddenChannels?.isHiddenChannel?.(channel)) {
            return true;
        }
    } catch (e) {
        // Ігноруємо помилки
...
function triggerFollow(userChannelId: string | null = getChannelId(settings.store.followUserId)) {
    if (settings.store.followUserId) {
        ...
        if (userChannelId) {
            ...
            // Повністю блокуємо вхід в приховані канали
            if (isHiddenChannel(channel)) {
                Toasts.show({
                    message: "Cannot follow user into hidden voice channel",
                    id: Toasts.genId(),
